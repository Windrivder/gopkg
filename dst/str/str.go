...
import (
	...
	"github.com/windrivder/gopkg/faker"
)
...
func Shuffle(str string) string {
	...
	for i, v := range faker.Perm(len(runes)) {
		s[i] = runes[v]
	}
	return string(s)
}
...
// JoinByPoint
func JoinByPoint(items ...string) string {
	return Join(items, ".")
}
...
func JoinAny(array interface{}, sep string) string {
	return strings.Join(cast.ToStringSlice(array), sep)
}
